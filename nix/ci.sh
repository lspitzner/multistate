...
function build-one {
  local ATTRPATH=$1
  # nix-build --no-out-link nix/all.nix -A "\"$ATTRPATH\".multistate.components.library"\
  #   2> >(tee "$OUTDIR/$ATTRPATH-1-build-lib.txt" >&2)
  # (($? == 0)) || { echo "$ATTRPATH: build src failed" >> "$SUMMARY"; return 1; }
  # nix-build --no-out-link nix/all.nix -A "\"$ATTRPATH\".multistate.components.tests.multistate-test"\
  #   2> >(tee "$OUTDIR/$ATTRPATH-2-build-test.txt" >&2)
  # (($? == 0)) || { echo "$ATTRPATH: build test failed" >> "$SUMMARY"; return 1; }
  OUT=$(nix-build -o "$OUTDIR/$ATTRPATH-test-result.txt" nix/all.nix -A "\"$ATTRPATH\".multistate.checks.multistate-test"\
